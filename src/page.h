...
*/
  struct tx_details
  {
    ...
    uint64_t create_feedback_token_inputs;
    ...
    get_mstch_map() const
    {
      ...
      const double &xmr_amount = SAFEX_AMOUNT(fee);
      ...
      if (!input_key_imgs.empty())
      {
        ...
        mixin_str = std::to_string(cash_mixin_no);
  ...
  class page
  {
    ...
    public:
                ...
                tx_context_cache(1000)
      {
        ...
        template_file["css_styles"] = safexeg::read(TMPL_CSS_STYLES);
        template_file["header"] = safexeg::read(TMPL_HEADER);
        template_file["footer"] = get_footer();
        template_file["index2"] = get_full_page(safexeg::read(TMPL_INDEX2));
        template_file["mempool"] = safexeg::read(TMPL_MEMPOOL);
        template_file["altblocks"] = get_full_page(safexeg::read(TMPL_ALTBLOCKS));
        template_file["mempool_error"] = safexeg::read(TMPL_MEMPOOL_ERROR);
        template_file["mempool_full"] = get_full_page(template_file["mempool"]);
        template_file["block"] = get_full_page(safexeg::read(TMPL_BLOCK));
        template_file["tx"] = get_full_page(safexeg::read(TMPL_TX));
        template_file["my_outputs"] = get_full_page(safexeg::read(TMPL_MY_OUTPUTS));
        template_file["rawtx"] = get_full_page(safexeg::read(TMPL_MY_RAWTX));
        template_file["checkrawtx"] = get_full_page(safexeg::read(TMPL_MY_CHECKRAWTX));
        template_file["pushrawtx"] = get_full_page(safexeg::read(TMPL_MY_PUSHRAWTX));
        template_file["rawkeyimgs"] = get_full_page(safexeg::read(TMPL_MY_RAWKEYIMGS));
        template_file["rawoutputkeys"] = get_full_page(safexeg::read(TMPL_MY_RAWOUTPUTKEYS));
        template_file["checkrawkeyimgs"] = get_full_page(safexeg::read(TMPL_MY_CHECKRAWKEYIMGS));
        template_file["checkoutputkeys"] = get_full_page(safexeg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
        template_file["address"] = get_full_page(safexeg::read(TMPL_ADDRESS));
        template_file["search_results"] = get_full_page(safexeg::read(TMPL_SEARCH_RESULTS));
        template_file["tx_details"] = safexeg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
        template_file["tx_table_header"] = safexeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
        template_file["tx_table_row"] = safexeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");

        if (enable_js)
        {
          // JavaScript files
          template_file["jquery.min.js"] = safexeg::read(JS_JQUERY);
          template_file["crc32.js"] = safexeg::read(JS_CRC32);
          template_file["crypto.js"] = safexeg::read(JS_CRYPTO);
          template_file["cn_util.js"] = safexeg::read(JS_CNUTIL);
          template_file["base58.js"] = safexeg::read(JS_BASE58);
          template_file["nacl-fast-cn.js"] = safexeg::read(JS_NACLFAST);
          template_file["sha3.js"] = safexeg::read(JS_SHA3);
          template_file["config.js"] = safexeg::read(JS_CONFIG);
          template_file["biginteger.js"] = safexeg::read(JS_BIGINT);
     ...
     */
      ...
      index2(uint64_t page_no = 0, bool refresh_page = false)
      {
        ...
        mstch::map context{
                ...
                {"height",                    height},
                {"server_timestamp",          safexeg::timestamp_to_str_gm(local_copy_server_timestamp)},
        ...
        while (i >= start_height)
        {
          ...
          double blk_size =
                  static_cast<double>(core_storage->get_db().get_block_size(
                          i)) / 1024.0;
        ...
        //double blk_size_median = safexeg::calc_median(blk_sizes.begin(), blk_sizes.end());
        ...
        if (CurrentBlockchainStatus::is_thread_running())
        {
          ...
          string emission_coinbase = safex_amount_to_str(current_values.coinbase, "{:0.3f}");
          string emission_fee = safex_amount_to_str(current_values.fee, "{:0.3f}");
     ...
     */
      ...
      mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
      {
        ...
        for (size_t i = 0; i < no_of_mempool_tx; ++i)
        {
          ...
          txs.push_back(mstch::map{
                  ...
                  {"cash_inputs",      mempool_tx.xmr_inputs_str},
                  {"cash_outputs",     mempool_tx.xmr_outputs_str},
      ...
      show_block(uint64_t _blk_height)
      {
        ...
        string blk_timestamp = safexeg::timestamp_to_str_gm(blk.timestamp);
        ...
        for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
        {
          ...
          //string mixin_times_scale = safexeg::timestamps_time_scale(mixin_timestamps,
          //                                                        server_timestamp);
        ...
        context["sum_fees"]
                = safexeg::safex_amount_to_str(sum_fees, "{:0.6f}", false);

        // get xmr in the block reward
        context["blk_reward"]
                = safexeg::safex_amount_to_str(txd_coinbase.cash_outputs - sum_fees, "{:0.6f}");
      ...
      show_block(string _blk_hash)
      {
        ...
        if (!safexeg::parse_str_secret_key(_blk_hash, blk_hash))
        {
      ...
      show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0)
      {
        ...
        if (!mcore->get_tx(tx_hash, tx))
        {
          ...
          if (!found_txs.empty())
          {
            ...
            blk_timestamp = safexeg::timestamp_to_str_gm(tx_recieve_timestamp);
                      ...
                      bool tx_prove = false)
      {
        ...
        if (!safexeg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
        {
        ...
        if (tx_blk_found)
        {
          ...
          blk_timestamp = safexeg::timestamp_to_str_gm(blk.timestamp);
        ...
        string server_time_str = safexeg::timestamp_to_str_gm(server_timestamp, "%F");
        ...
        for (auto &outp: txd.output_pub_keys)
        {
          ...
          //get_displayable_key(outp.first)
          bool mine_output = (get_public_key(outp.first) == tx_pubkey);
          ...
          if (!mine_output && txd.additional_pks.size() == txd.output_pub_keys.size())
          {
            ...
            mine_output = (get_public_key(outp.first) == tx_pubkey);
...
//            }
        ...
        vector<txin_to_key> input_key_imgs = safexeg::get_key_images(tx);
        ...
        for (const txin_to_key &in_key: input_key_imgs)
        {
          ...
          inputs.push_back(mstch::map{
                  ...
                  {"key_image_amount", safexeg::safex_amount_to_str(in_key.amount)},
          ...
          for (const uint64_t &abs_offset: absolute_offsets)
          {
            ...
            try
            {
              ...
              tx_out_idx = core_storage->get_db()
                      .get_output_tx_and_index(in_key.amount, abs_offset, tx_out_type::out_cash);
            }
            catch (const OUTPUT_DNE &e)
            {
            ...
            public_key mixin_tx_pub_key
                    = safexeg::get_tx_pub_key_from_received_outs(mixin_tx);
            ...
            output_pub_keys = safexeg::get_ouputs_tuple(mixin_tx);
...
//                        }
        ...
        context["sum_mixin_xmr"] = safexeg::safex_amount_to_str(
      ...
      show_checkrawtx(string raw_tx_data, string action)
      {
        ...
        string data_prefix = safexeg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
        ...
        if (unsigned_tx_given)
        {
          ...
          if (r)
          {
            ...
            for (const ::tools::wallet::tx_construction_data &tx_cd: exported_txs.txes)
            {
              ...
              for (size_t i = 0; i < no_of_sources; ++i)
              {
                ...
                try
                {
                  ...
                  real_toi = core_storage->get_db()
                          .get_output_tx_and_index(tx_source_amount,
                                                   index_of_real_output, tx_out_type::out_cash);
                }
                catch (const OUTPUT_DNE &e)
                {
                ...
                public_key real_out_pub_key = get_public_key(real_txd.output_pub_keys[tx_source.real_output_in_tx_index].first);
                ...
                for (const tx_source_entry::output_entry &oe: tx_source.outputs)
                {
                  ...
                  try
                  {
                    ...
                    toi = core_storage->get_db()
                            .get_output_tx_and_index(tx_source_amount, oe.first, tx_out_type::out_cash);
                  }
                  catch (OUTPUT_DNE &e)
                  {
                  ...
                  public_key out_pub_key = get_public_key(txd.output_pub_keys[toi.second].first);
              ...
              tx_cd_data.insert({"sum_outputs_amounts",
                                 safexeg::safex_amount_to_str(sum_outputs_amounts)});
              ...
              tx_cd_data["min_mix_time"] = safexeg::timestamp_to_str_gm(min_mix_timestamp);
              tx_cd_data["max_mix_time"] = safexeg::timestamp_to_str_gm(max_mix_timestamp);
        ...
        else
        {
          ...
          ::tools::wallet::signed_tx_set signed_txs;
          ...
          std::vector<tools::wallet::pending_tx> ptxs = signed_txs.ptx;

          context.insert({"txs", mstch::array{}});

          for (tools::wallet::pending_tx &ptx: ptxs)
          {
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
              ...
              if (parse_amount(output_amount, out_amount_str))
              {
                if (output_amount == 0)
                {
                  out_amount_str = safexeg::safex_amount_to_str(real_ammounts.at(i));
                }
              }
            }
            ...
            for (const tx_source_entry &tx_source: ptx.construction_data.sources)
            {
              ...
              try
              {
                // get tx of the real output
                real_toi = core_storage->get_db()
                        .get_output_tx_and_index(tx_source_amount, index_of_real_output, tx_out_type::out_cash);
              }
              catch (const OUTPUT_DNE &e)
              {
              ...
              public_key real_out_pub_key
                      = get_public_key(real_txd.output_pub_keys[tx_source.real_output_in_tx_index].first);
            ...
            for (mstch::node &input_node: inputs)
            {
              ...
              amount = safexeg::safex_amount_to_str(real_amounts.at(input_idx));
      ...
      show_pushrawtx(string raw_tx_data, string action)
      {
        ...
        {
          tools::wallet::pending_tx &ptx = ptx_vector.back();
          ...
          for (auto const &tx_in: txd.input_key_imgs)
          {
            auto key_image = get_key_image(tx_in);
            if (core_storage->have_tx_keyimg_as_spent(key_image))
              key_images_spent.push_back(key_image);
          }
      ...
      show_checkrawkeyimgs(string raw_data, string viewkey_str)
      {
        ...
        if (!safexeg::parse_str_secret_key(viewkey_str, prv_view_key))
        {
        ...
        string data_prefix = safexeg::make_printable(decoded_raw_data.substr(0, magiclen));
        ...
        decoded_raw_data = safexeg::decrypt(
        ...
        context.insert({"address", REMOVE_HASH_BRAKETS(
                safexeg::print_address(address_info, nettype))});
      ...
      show_checkcheckrawoutput(string raw_data, string viewkey_str)
      {
        ...
        for (const tools::wallet::transfer_details &td: outputs)
        {
          ...
          if (td.is_rct())
          {
            ...
            public_key tx_pub_key = safexeg::get_tx_pub_key_from_received_outs(tx);
        ...
        if (total_xmr > 0)
        {
          ...
          context["total_xmr"] = safexeg::safex_amount_to_str(total_xmr);
        }
      ...
      search(string search_text)
      {
        ...
        if (search_str_length == 95)
        {
          ...
          if (!safexeg::parse_str_address(search_text, address_info, nettype_addr))
          {
      ...
      show_address_details(const address_parse_info &address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
      {
        ...
        string address_str = safexeg::print_address(address_info, nettype);
      ...
      search_txs(vector<transaction> txs, const string &search_text)
      {
        ...
        for (const transaction &tx: txs)
        {
          ...
          auto it1 = find_if(begin(txd.input_key_imgs), end(txd.input_key_imgs),
                             [&](const displayable_input &key_img)
                             {
                               return pod_to_hex(get_key_image(key_img)) == search_text;
                             });
          ...
          auto it2 = find_if(begin(txd.output_pub_keys), end(txd.output_pub_keys),
                             [&](const pair<displayable_output, uint64_t> &tx_out_pk)
                             {
                               return pod_to_hex(get_public_key(tx_out_pk.first)) == search_text;
                             });
        ...
        for (const pair<string, vector<string>> &found_txs: all_possible_tx_hashes)
        {
          ...
          if (!found_txs.second.empty())
          {
            ...
            for (const string &tx_hash: found_txs.second)
            {
              ...
              txd_map.insert({"timestamp", safexeg::timestamp_to_str_gm(blk_timestamp)});
     ...
     */
      ...
      json_transaction(string tx_hash_str)
      {
        ...
        string blk_timestamp_utc = safexeg::timestamp_to_str_gm(tx_timestamp);

        // get the current blockchain height. Just to check
        uint64_t bc_height = core_storage->get_current_blockchain_height();

        tx_details txd = get_tx_details(
                tx, is_coinbase_tx, block_height, bc_height);
        ...
        for (const auto &output: txd.output_pub_keys)
        {
          outputs.push_back(json{
                  {"public_key", pod_to_hex(get_public_key(output.first))},
        ...
        for (auto const &in_key: txd.input_key_imgs)
        {

          // get absolute offsets of mixins
          std::vector<uint64_t> absolute_offsets
                  = cryptonote::relative_output_offsets_to_absolute(
                          get_key_offsets(in_key));

          // get public keys of outputs used in the mixins that match to the offests
          std::vector<output_data_t> outputs;
          std::vector<output_data_t> token_outputs;

          auto const amount = get_cash_amount(in_key);
     ...
     */
      ...
      json_block(string block_no_or_hash)
      {
        ...
        else if (block_no_or_hash.length() == 64)
        {
          ...
          if (!safexeg::parse_str_secret_key(block_no_or_hash, blk_hash))
          {
        ...
        j_data = json{
                ...
                {"timestamp_utc",  safexeg::timestamp_to_str_gm(blk.timestamp)},
                {"block_height",   block_height},
                {"size",           blk_size},
                {"txs",            j_txs},
                   ...
                   bool tx_prove = false)
      {
        ...
        for (auto &outp: txd.output_pub_keys)
        {
          ...
          j_outptus.push_back(json{
                  {"output_pubkey", pod_to_hex(get_public_key(outp.first))},
                  {"amount",        outp.second},
                  {"match",         mine_output},
    ...
    */
      ...
      json_emission()
      {
        ...
        else
        {
          ...
          string emission_blk_no = std::to_string(current_values.blk_no - 1);
          string emission_coinbase = safex_amount_to_str(current_values.coinbase, "{:0.3f}");
          string emission_fee = safex_amount_to_str(current_values.fee, "{:0.3f}", false);
      ...
      json_version()
      {
        ...
        j_data = json{
                ...
                {"safex_version_full",  string{SAFEX_VERSION_FULL}},
    ...
    private:
              ...
              string &error_msg)
      {
        ...
        for (auto it = txs_begin; it != txs_end; ++it)
        {
          ...
          for (auto &outp: txd.output_pub_keys)
          {
            ...
            // check if generated public key matches the current output's key
            bool mine_output = (key == tx_pubkey);
            bool with_additional = false;
            if (!mine_output && txd.additional_pks.size() == txd.output_pub_keys.size())
            {
              derive_public_key(additional_derivations[output_idx],
      ...
      get_tx_json(const transaction &tx, const tx_details &txd)
      {
        ...
        json j_tx{
                {"tx_hash",      pod_to_hex(txd.hash)},
                {"tx_fee",       txd.fee},
                {"mixin",        txd.cash_mixin_no},
                {"tx_size",      txd.size},
                {"cash_outputs", txd.cash_outputs},
                {"cash_inputs",  txd.cash_inputs},
      ...
      construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
      {
        ...
        double payed_for_kB = SAFEX_AMOUNT(txd.fee) / tx_size;
        ...
        uint64_t input_idx{0};
        uint64_t inputs_safex_cash_sum{0};
        uint64_t inputs_token_sum{0};
        ...
        for (auto const &in_key: txd.input_key_imgs)
        {
          ...
          auto const cash_amount = get_cash_amount(in_key);
          ...
          {
            string out_msg = fmt::format("Outputs with amount {:d} and token amount {:d} do not exist and indexes ", cash_amount, token_amount);
          ...
          inputs.push_back(mstch::map{
                  ...
                  {"is_token",           is_token},
                  {"token_amount",       safexeg::safex_amount_to_str(token_amount, "{:0.0f}")},
                  {"cash_amount",        safexeg::safex_amount_to_str(cash_amount)},
                  {"bitcoin_hash",       static_cast<bool>(bitcoin_hash)},
                  {"bitcoin_hash_value", bitcoin_hash ? epee::string_tools::pod_to_hex(*bitcoin_hash) : std::string{}},
          ...
          if (is_token)
            inputs_token_sum += token_amount;
          else
            inputs_safex_cash_sum += cash_amount;

          vector<uint64_t> mixin_timestamps;
          // get reference to mixins array created above
          auto &mixins = boost::get<mstch::array>(boost::get<mstch::map>(inputs.back())["mixins"]);
          ...
          for (const uint64_t &i: absolute_offsets)
          {
            ...
            if (detailed_view)
            {
              ...
              mixins.push_back(mstch::map{
                      ...
                      {"mix_mixin_no",   mixin_txd.cash_mixin_no},
      ...
      )
      {
        ...
        {
          // get mixins in time scale for visual representation
          pair<string, double> mixin_times_scale = safexeg::timestamps_time_scale(
                  mixn_timestamps,
                  max_mix_timestamp,
                  170,
                  min_mix_timestamp);
                     ...
                     uint64_t bc_height = 0)
      {
        ...
        txd.create_feedback_token_inputs = sum_data[20];
        ...
        else if (!txd.additional_pks.empty())
        {
      ...
      are_absolute_offsets_good(
              ...
              displayable_input const &in_key)
      {
        // before proceeding with geting the outputs based on the cash_amount and absolute offset
        // check how many outputs there are for that cash_amount
        ...
        {
          auto const amount = output_type == tx_out_type::out_token ? get_token_amount(in_key) : get_cash_amount(in_key);
          uint64_t no_outputs = core_storage->get_db().get_num_outputs(amount, output_type);
          int offset_idx{-1};
          for (auto o: absolute_offsets)
          {
            ...
            if (o >= no_outputs)
            {
              ...
              cerr << "Absolute offset (" << o << ") of an output in a key image " << pod_to_hex(k_image) << " (ring member no: " << offset_idx << ") "
                   << "for amount " << amount << " is too large. There are only " << no_outputs << " such outputs!\n";
              continue;
            }
          }
        }
      ...
      get_footer()
      {
        ...
        string footer_html = mstch::render(safexeg::read(TMPL_FOOTER), footer_context);
